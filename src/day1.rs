...
pub fn run() {
    let file_path = "data/test_puzzle_input.txt";
...
fn get_password_part_1(file_name: &str) -> u32 {
    ...
    for line in fs::read_to_string(file_name).unwrap().lines() {
        ...
        let rotation_value: i32 = match (left, right) {
            ...
            (Some(l), None) => - (l.parse::<i32>().expect("Invalid string, should be a number")),
...
fn get_password_part_2(file_name: &str) -> i64 {
    ...
    for line in fs::read_to_string(file_name).unwrap().lines() {
        ...
        if rotation_value >= 0 {
            ...
            num_times_passed_zero = (((current_position - 100) + rotation_value) / 100).abs()
        }
        println!("times passed zero: {num_times_passed_zero}");
